  ...
  <aside class="group relative flex w-full flex-col gap-y-6">
    <div class="pointer-events-none relative aspect-[16/9] w-full overflow-hidden rounded-lg ring-1 ring-border">
      <NuxtImg
        ...
        height="140"
        width="250"
        :loading="post.highlight === true ? 'eager' : 'lazy'"
      />
    </div>
    <div class="flex flex-1 flex-col justify-between">
      <div class="flex-1">
        <div class="mb-3" v-if="post.category_text">
          <span  class="mr-2 inline-flex items-center rounded-md px-2 py-1 text-xs font-medium text-primary ring-1 ring-inset ring-primary ">
            {{ post.category_text }}
          </span>
        </div>
      ...
      <div class="relative mt-4 flex items-center gap-x-3">
        <div class="flex items-center space-x-4">
          <NuxtTime 
            ...
            class="relative text-sm font-medium text-muted-foreground after:absolute after:ml-1.5 after:text-muted-foreground"
          />
